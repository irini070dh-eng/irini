...
const Menu: React.FC = () => {
  ...
  return (
    <section id="menu" ref={scrollRef} className="py-24 px-4 bg-zinc-950 relative min-h-screen">
      {/* Background radial gradient for depth */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full bg-[radial-gradient(circle_at_50%_0%,_rgba(212,175,55,0.05)_0%,_transparent_70%)] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col xl:flex-row xl:items-end justify-between mb-24 gap-12">
          <div className="space-y-6 max-w-2xl animate-reveal">
            <div className="flex items-center gap-4">
              ...
              <span className="text-gold-400 uppercase tracking-[0.6em] text-[10px] font-bold">
                {t.familyBusiness}
              </span>
            </div>
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`relative px-8 py-5 rounded-2xl text-[10px] font-bold uppercase tracking-[0.25em] transition-all duration-400 overflow-hidden group/cat ${
                  activeCategory === cat 
                  ? 'text-zinc-950 scale-[1.08] shadow-[0_20px_40px_-15px_rgba(212,175,55,0.3)]' 
                  : 'text-zinc-500 hover:text-zinc-200 glass border border-zinc-800/50 hover:scale-105'
                }`}
              >
                {/* Active Background Slide */}
                <div className={`absolute inset-0 gold-bg transition-transform duration-500 ease-out ${activeCategory === cat ? 'translate-y-0' : 'translate-y-full'}`} />
        ...
        >
          {displayItems.map((item, index) => (
            <div 
              key={item.id} 
              className={`group relative glass rounded-[3rem] overflow-hidden border border-zinc-900/50 hover:border-gold-400/30 transition-all duration-700 hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.8)] opacity-0 animate-reveal stagger-${(index % 6) + 1}`}
            >
              {/* Image Container */}
              <div className="relative aspect-[4/5] overflow-hidden">
                ...
                <div className="absolute top-8 right-8 z-20">
                  <div className="glass border border-white/10 backdrop-blur-xl px-6 py-3 rounded-2xl shadow-2xl">
                    <span className="text-gold-400 font-bold tracking-tight text-lg">€{item.price.toFixed(2)}</span>
                  </div>
                </div>
                ...
                <div className="absolute inset-x-0 bottom-0 p-10 z-20 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                  <h3 className="text-3xl font-serif font-bold text-white mb-2 group-hover:text-gold-400 transition-colors">
                    {item.names[language]}
                  </h3>
                  <p className="text-zinc-400 text-sm line-clamp-2 font-light leading-relaxed mb-8 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
                  ...
                  <button 
                    ...
                    className="w-full py-5 gold-bg text-zinc-950 rounded-[1.5rem] text-[10px] font-bold uppercase tracking-[0.3em] transition-all active:scale-95 flex items-center justify-center gap-3 shadow-2xl translate-y-10 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 duration-500 delay-200"
                  >
